...
def val(net, criterion):
    ...
    for raw_pred, pred, gt in zip(raw_preds, sim_preds, cpu_texts_decode):
        # 强制调整成6位
        if len(pred) < 6:
            pred = pred + '0'*(6-len(pred))
        elif len(pred) > 6:
            pred = pred[:6]
        if pred != gt :
            print('%-20s => %-20s, gt: %-20s' % (raw_pred, pred, gt))

    accuracy = n_correct / float(200)
...
if __name__ == "__main__":
    ...
    for epoch in range(params.nepoch):
        ...
        while i < len(train_loader):
            cost, correct_ = train(crnn, criterion, optimizer, train_iter)
            loss_avg.add(cost)
            correct += correct_
            i += 1

        print('[%d/%d] Loss: %f Accuracy: %f' %
              (epoch, params.nepoch, loss_avg.val(), correct / float(640)))
        loss_avg.reset()
        ...
        if epoch % params.valInterval == 0:
            ...
            if accuracy > best_accuracy:
                ...
                torch.save(crnn.state_dict(), '{0}/best.pth'.format(params.expr_dir))
                print("epoch %d save best once" % epoch )
